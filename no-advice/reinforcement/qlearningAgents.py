...
class PacmanQAgent(QLearningAgent):
    ...
    def __init__(self, epsilon=0.15, gamma=0.8, alpha=0.2, numTraining=0, **args):
        """
...
class ApproximateQAgent(PacmanQAgent):
    ...
    def final(self, state):
        ...
        if self.episodesSoFar == self.numTraining:
            ...
            w = csv.writer(open(('/home/sunny/Desktop/intern_30_07/solvedpacman/no-advice/reinforcement/output/output' + '-'.join([str(t) for t in time.localtime()[1:6]]) + '.csv'), "w"))           #writing learned weights in file
            for key, val in self.weights.items():
                w.writerow([key, val])
            pass
            """
            import time, cPickle
            fname = ('recorded-weights')
            f = file(fname, 'w')
            #components = {'layout': layout, 'actions': game.moveHistory}
            l1=[feature for feature in self.weights.keys()]
            l2=[self.weights[feature] for feature in self.weights.keys()]
            l1.astype(str)
            l2=np.asarray(l2)
            comb = np.stack((l1,l2), axis = 1 )
...
class NeuralNetQAgent(PacmanQAgent):
    ...
    def update(self, state, action, nextState, reward):
        ...
        y = reward + (self.discount * self.getValue(nextState))
...
class NeuralNetwork:
    def __init__(self, state):
        ...
        self.featExtractor = util.lookup('AllExtractor', globals())()
        tempaction = 'East'
        #reshaped_state = self.reshape(state, tempaction)
        f = self.featExtractor.getFeatures(state, tempaction)
        reshaped_state = f.values()
        print "input shape", len(reshaped_state)
        
        self.model = Sequential()
        #self.model.add(Dense(50, init='lecun_uniform', input_shape=(len(reshaped_state),)))
        self.model.add(Dense(50, init='lecun_uniform', input_shape = (5,)))
        self.model.add(Activation('linear'))

        self.model.add(Dense(25, init='lecun_uniform'))
        self.model.add(Activation('linear'))
    ...
    def predict(self, state, action):
        ...
        reshaped_state = f.values()
        inp = np.asarray(reshaped_state)
        inp = inp.reshape((1,5))
        print inp.shape
        #return self.model.predict(inp, batch_size=1)[0][0]
    ...
    def update(self, state, action, y):
        ...
        y = np.asarray(y)
        print "y",y
        self.model.fit(inp, y, batch_size=1, nb_epoch=1, verbose=1)
