...
i = movavg-1
while i <= len(epi1):
	avg1.append(sum(win1[i+1-movavg:i]) )
	avg2.append(sum(win2[i+1-movavg:i]) )
...
i = movavg-1
while i <= len(epi1):
	runningavg1.append(sum(win1[0:i]))
	runningavg2.append(sum(win2[0:i]))
...
plt.plot(avgepisode, avg2, 'k-', linewidth = 0.8,label='With advice')
...
plt.plot(avgepisode, runningavg2, 'k-', linewidth = 0.8,label='With advice')
